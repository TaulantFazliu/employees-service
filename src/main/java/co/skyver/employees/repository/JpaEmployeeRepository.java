...
public class JpaEmployeeRepository implements EmployeeRepository {
    ...
    public Optional<Employee> findByIdWithTitles(Long empNo) {
        TypedQuery<Employee> query = this.entityManager.createQuery("SELECT e FROM Employee e JOIN FETCH e.titles t WHERE e.empNo = :empNo ORDER BY t.toDate DESC", Employee.class).setParameter("empNo", empNo);
        return Optional.ofNullable(query.getSingleResult());
    }
